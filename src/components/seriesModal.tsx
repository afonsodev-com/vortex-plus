...
export function SeriesModal() {
  ...
  const saveToFirestore = async () => {
    ...
    try {
      const seriesId = title.replace(/\s+/g, '-').toLowerCase();
      ...
      toast({
        ...
        description: (
          <div className="flex justify-between items-center">
            <span>
              A série <strong className='text-blue-500'>{seriesId}</strong> foi salva com sucesso.
            </span>
      ...
      toast({
        title: "Erro!",
        description: "Ocorreu um erro ao salvar a série.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[800px]">
        ...
        <DialogFooter>
          <Button type="submit" onClick={saveToFirestore}>Salvar Série</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
